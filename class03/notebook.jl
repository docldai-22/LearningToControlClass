...
let
    ...
    ĉ = sol.zero
    println("Optimal shooting params (c1, c2) = ", ĉ)

    # ----- Reconstruct full optimal trajectories -----
    soltraj, utraj = rollout(ĉ; save_traj=true)
    ts  = soltraj.t
...
let
    ...
    u_star(t) = xdot_star(t)          # because  ẋ = u
    ...
    println("  state eqn (ẋ - ∂H/∂λ):   ", max_res_state)
